        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'siachenganpai',//当前表表名,
                        // sumColum : 'siachenganpai_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'siachenganpai_name',//分组字符串字段
                        types : 'siachenganpai_types',//分组下拉框字段
                    }
                }
                ...
                //         groupColumn: "siachenganpai_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
			...
			jieshubisai(id){
				...
				this.$confirm(`确定比赛已经结束吗?`, "提示", {
				    confirmButtonText: "确定",
				    cancelButtonText: "取消",
				    type: "warning"
				}).then(() => {
				    this.$http({
				        url: "siachenganpai/update",
				        method: "post",
				        data: data
				    }).then(({data}) => {
				        if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            siachenganpaiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "siachenganpai/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
