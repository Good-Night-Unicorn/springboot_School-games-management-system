        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('bisaixiangmu','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xiaoyunhuiguanli/upload/bisaixiangmuMuBan.xls"
                        >批量导入项目信息数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('bisaixiangmu','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="bisaixiangmuUploadSuccess"
                                :on-error="bisaixiangmuUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('bisaixiangmu','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入项目信息数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('bisaixiangmu','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "bisaixiangmu.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="bisaixiangmuShijian"
                                   header-align="center"
                                   label="工作时间">
                        <template slot-scope="scope">
                            {{scope.row.bisaixiangmuShijian}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="bisaixiangmuFuzhenr"
                                   header-align="center"
                                   label="负责人">
                        <template slot-scope="scope">
                            {{scope.row.bisaixiangmuFuzhenr}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('bisaixiangmu','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('bisaixiangmu','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('bisaixiangmu','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                id : null,
                bisaixiangmuName : null,
                bisaixiangmuPhoto : null,
                bisaixiangmuShijian : null,
                bisaixiangmuFuzhenr : null,
                bisaixiangmuPhone : null,
                bisaixiangmuTypes : null,
                jieshuTime : null,
                bisaixiangmuContent : null,
                bisaixiangmuDelete : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'bisaixiangmu',//当前表表名,
                        // sumColum : 'bisaixiangmu_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'bisaixiangmu_name',//分组字符串字段
                        types : 'bisaixiangmu_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "bisaixiangmu_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['bisaixiangmuDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "bisaixiangmu/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            bisaixiangmuUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "bisaixiangmu/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
