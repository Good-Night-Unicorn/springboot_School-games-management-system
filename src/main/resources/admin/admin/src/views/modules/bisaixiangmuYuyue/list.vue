<template>
    <div class="main-content">
        <el-dialog title="审核" :visible.sync="bisaixiangmuYuyueYesnoTypesVisible">
            <el-form :model="form">
                <input type="hidden" v-model="form.id">
                <el-form-item label="审核" >
                    <el-select v-model="form.bisaixiangmuYuyueYesnoTypes" placeholder="请选择审核状态">
                        <el-option label="同意" value="2"></el-option>
                        <el-option label="拒绝" value="3"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="审核回复">
                    <el-input type="textarea" v-model="form.bisaixiangmuYuyueYesnoText" placeholder="审核回复"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button @click="bisaixiangmuYuyueYesnoTypesVisible = false">取 消</el-button>
                <el-button type="primary" @click="bisaixiangmuYuyueYesnoTypesShenhe">审 核</el-button>
            </div>
        </el-dialog>
        <!-- 条件查询 -->
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '审核状态' : ''">
                        <el-select v-model="searchForm.bisaixiangmuYuyueYesnoTypes" placeholder="请选择审核状态">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in bisaixiangmuYuyueYesnoTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('bisaixiangmuYuyue','新增')"
                                type="success"
                                icon="el-icon-plus"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('bisaixiangmuYuyue','删除')"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('bisaixiangmuYuyue','报表')"
                                type="success"
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('bisaixiangmuYuyue','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xiaoyunhuiguanli/upload/bisaixiangmuYuyueMuBan.xls"
                        >批量导入项目报名数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('bisaixiangmuYuyue','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="bisaixiangmuYuyueUploadSuccess"
                                :on-error="bisaixiangmuYuyueUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('bisaixiangmuYuyue','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入项目报名数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('bisaixiangmuYuyue','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "bisaixiangmuYuyue.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                          ...
                          v-if="isAuth('bisaixiangmuYuyue','查看')"
                          :data="dataList"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="bisaixiangmuYuyueYesnoTypes"
                                      header-align="center"
                                      label="审核状态">
                        <template slot-scope="scope">
                            {{scope.row.bisaixiangmuYuyueYesnoValue}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="bisaixiangmuYuyueYesnoText"
                                      header-align="center"
                                      label="审核回复">
                        <template slot-scope="scope">
                            <span v-if="scope.row.bisaixiangmuYuyueYesnoText != null &&scope.row.bisaixiangmuYuyueYesnoText.length>10">
                                {{scope.row.bisaixiangmuYuyueYesnoText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.bisaixiangmuYuyueYesnoText}}
                            </span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('bisaixiangmuYuyue','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('bisaixiangmuYuyue','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('bisaixiangmuYuyue','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('bisaixiangmuYuyue','审核') && scope.row.bisaixiangmuYuyueYesnoTypes == 1 " type="primary" icon="el-icon-thumb" size="mini" @click="openYesnoTypes(scope.row.id)">审核</el-button>
                            <el-button v-if="isAuth('bisaixiangmuYuyue','审核') && scope.row.bisaixiangmuYuyueYesnoTypes == 2 " type="primary" icon="el-icon-thumb" size="mini" @click="saichenganpai(scope.row.bisaixiangmuId,scope.row.yonghuId)">赛程安排</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              bisaixiangmuYuyueYesnoTypesSelectSearch : [],
            form:{
                id : null,
                bisaixiangmuId : null,
                yonghuId : null,
                bisaixiangmuYuyueYesnoTypes : null,
                bisaixiangmuYuyueYesnoText : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            bisaixiangmuYuyueYesnoTypesVisible : false,
            ...
            json_fields: {
                //级联表字段
                     ...
                     '用户邮箱': 'yonghuEmail',
                //本表字段
                     '审核状态': "bisaixiangmuYuyueYesnoValue",
                     '审核回复': "bisaixiangmuYuyueYesnoText",
                     '申请时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'bisaixiangmu_yuyue',//当前表表名,
                        sumColum : 'bisaixiangmu_yuyue_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'bisaixiangmu_yuyue_name',//分组字符串字段
                        // types : 'bisaixiangmu_yuyue_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "bisaixiangmu_yuyue_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                if (this.searchForm.bisaixiangmuYuyueYesnoTypes!= '' && this.searchForm.bisaixiangmuYuyueYesnoTypes!= undefined) {
                    params['bisaixiangmuYuyueYesnoTypes'] = this.searchForm.bisaixiangmuYuyueYesnoTypes
                }
                        
                params['bisaixiangmuYuyueDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "bisaixiangmuYuyue/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=bisaixiangmu_yuyue_yesno_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.bisaixiangmuYuyueYesnoTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
			...
			saichenganpai(bisaixiangmuId,yonghuId){
				this.$router.push(
					{
            ...
            // 导入功能上传文件成功后调用导入方法
            bisaixiangmuYuyueUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "bisaixiangmuYuyue/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            bisaixiangmuYuyueUploadError(data){
                ...
                _this.form.id = null;
                _this.form.id = id;
                            _this.form.bisaixiangmuYuyueYesnoText = null;
                _this.form.bisaixiangmuYuyueYesnoTypes = "请选择审核状态";
                _this.bisaixiangmuYuyueYesnoTypesVisible = true;
            },

            bisaixiangmuYuyueYesnoTypesShenhe() {
                let _this = this;
                if(_this.form.bisaixiangmuYuyueYesnoTypes == "请选择审核状态"){
                    _this.$message.error("请选择审核状态");return false;
                }
                if(_this.form.bisaixiangmuYuyueYesnoText == null || _this.form.bisaixiangmuYuyueYesnoText == ""){
                    _this.$message.error("请输入审核回复");return false;
                }
                this.$http({
                    url:`bisaixiangmuYuyue/shenhe`,
                    method: "post",
                    data: _this.form
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.$message({
                            ...
                            duration: 1500,
                            onClose: () => {
                                _this.bisaixiangmuYuyueYesnoTypesVisible = false;
                                _this.search();
                            }
                        });
                    } else {
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "bisaixiangmuYuyue/update",
                        method: "post",
                        data: {
                            id:id,
//                            bisaixiangmuYuyueTypes:bisaixiangmuYuyueTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
